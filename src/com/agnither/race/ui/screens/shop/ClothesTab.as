...
public class ClothesTab extends AbstractView {
    ...
    private var _items: Vector.<ClothTile>;

    public function ClothesTab(refs:CommonRefs, player: Player) {
        _player = player;
        super(refs);
    }

    override protected function initialize():void {
        _items = new <ClothTile>[];
        for (var i:* in ClothVO.CLOTHES) {
            var tile: ClothTile = new ClothTile(_refs, ClothVO.CLOTHES[i]);
    ...
    private function handleUpdate(e: Event):void {
        for (var i:int = 0; i < _items.length; i++) {
            var tile: ClothTile = _items[i];
            tile.update(_player.cloth == tile.id, _player.hasCloth(tile.id));
        }
    }
